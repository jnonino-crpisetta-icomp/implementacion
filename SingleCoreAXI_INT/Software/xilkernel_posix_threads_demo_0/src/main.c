...
void* master_thread(void *arg)
{
    ...
    //Lanzamiendo de los hilos escritores
		...
		status = pthread_create(&hilo_1, &attr, (void*)hilo_escritor_1, 0);
    	status = pthread_create(&hilo_2, &attr, (void*)hilo_escritor_2, 0);
    	status = pthread_create(&hilo_3, &attr, (void*)hilo_escritor_3, 0);
...
/*		//Pedir T0
		...
		//Ver si T0 se ejecuto
			...
			while ( !(disparos_ya_ejecutados & comprobacion_t_cero) )
			{ disparos_ya_ejecutados = *(leer_disparos_ejecutados);	}
		...
		//Ver si T1 se ejecuto
			...
			while ( !(disparos_ya_ejecutados & comprobacion_t_uno))
			{ disparos_ya_ejecutados = *(leer_disparos_ejecutados);	}
...
/*		//Pedir T2
		...
		//Ver si T2 se ejecuto
			disparos_ya_ejecutados = *(leer_disparos_ejecutados);
			while ( !(disparos_ya_ejecutados & comprobacion_t_dos))
			{	disparos_ya_ejecutados = *(leer_disparos_ejecutados);	}
	...
	//		xil_printf("%d",numero_de_hilo_dos);
			...
			*(error_addr) = 0xFFFFFFFF;
			disparos_ya_ejecutados = *(leer_disparos_ejecutados);
			while ( !(disparos_ya_ejecutados & comprobacion_t_tres) )
			{	disparos_ya_ejecutados = *(leer_disparos_ejecutados);	}
...
/*		//Pedir T2
		...
		//Ver si T2 se ejecuto
			disparos_ya_ejecutados = *(leer_disparos_ejecutados);
			while ( !(disparos_ya_ejecutados & comprobacion_t_cuatro))
			{	disparos_ya_ejecutados = *(leer_disparos_ejecutados);	}
...
//			xil_printf("%d",numero_de_hilo_tres);
			...
			*(error_addr) = 0xFFFFFFFF;
			disparos_ya_ejecutados = *(leer_disparos_ejecutados);
			while ( !(disparos_ya_ejecutados & comprobacion_t_cinco) )
			{	disparos_ya_ejecutados = *(leer_disparos_ejecutados);	}
